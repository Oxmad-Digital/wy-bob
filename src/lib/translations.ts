export type Locale = 'fr' | 'en'

export type HistoireSection = {
  kicker?: string
  title?: string
  paragraphs: string[]
  quote?: string
  paragraphsAfterQuote?: string[]
  closing?: string
  cta?: string
}

export type ConceptSection = {
  title?: string
  paragraphs: string[]
  closing?: string
}

export const translations = {
  fr: {
    nav: {
      home: 'Notre bob',
      histoire: 'Notre Histoire',
      concept: 'Notre concept',
      galerie: 'Galerie',
      contact: 'Contacts',
      panier: 'Panier',
      login: 'Se connecter',
      register: 'Créer un compte',
      dashboard: 'Mon compte',
      logout: 'Se déconnecter',
    },
    footer: {
      legal: 'Mentions légales',
      cgv: 'CGV',
      privacy: 'Politique de confidentialité',
      madeByPrefix: 'Réalisé par ',
      manageCookies: 'Gérer les cookies',
    },
    cookieConsent: {
      message: "Nous utilisons des cookies essentiels au fonctionnement du site et, avec votre accord, des cookies analytiques pour mesurer l'audience. Vous pouvez modifier votre choix à tout moment.",
      customize: 'Personnaliser',
      rejectAll: 'Refuser tout',
      acceptAll: 'Tout accepter',
      settingsTitle: 'Préférences cookies',
      essentialTitle: 'Cookies essentiels',
      essentialDesc: 'Nécessaires au fonctionnement du site (panier, session, authentification). Toujours actifs.',
      analyticsTitle: 'Cookies analytiques',
      analyticsDesc: "Google Analytics — mesure d'audience anonymisée. Déposés uniquement avec votre accord.",
      save: 'Enregistrer mes choix',
      privacyLink: 'En savoir plus dans notre politique de confidentialité',
      close: 'Fermer',
    },
    home: {
      tagline: "Pensé dans les moindres détails, le WYBOB allie caractère, confort et savoir-faire artisanal.",
      description: "Pensé pour les journées lumineuses et les escapades d'été, le WYBOB Soleil apporte une élégance naturelle à toutes vos silhouettes.",
      colors: {
        Blue: 'Bleu',
        White: 'Blanc',
        Yellow: 'Jaune',
        Red: 'Rouge',
      },
      colorStories: {
        Jaune: { name: 'MASOANDRO', text: "La lumière. Une couleur solaire qui attire les regards et apporte une touche de bonne humeur à chaque instant." },
        Bleu:  { name: 'RANO',       text: "L'horizon. Inspiré du ciel et de l'océan, un bleu qui évoque la liberté, le calme et l'évasion." },
        Rouge: { name: 'AFO',        text: "L'intensité. Une couleur vibrante qui affirme votre présence avec caractère et confiance." },
        Blanc: { name: 'RAHONA',     text: "L'essentiel. Une couleur intemporelle qui traverse les saisons et laisse toute la place à votre histoire." },
        Barea: { name: '',          text: "Une édition signature imaginée pour rendre hommage aux racines de WYBOB et célébrer l'héritage, les couleurs et le savoir-faire artisanal de Madagascar." },
      },
      colorLabel: 'Couleur actuelle :',
      sectionDescription: 'Description',
      sectionColor: 'Couleur',
      sectionFeatures: 'Caractéristiques',
      sectionCare: 'Entretien et lavage',
      features: [
        'Matière : 100% coton BIO tissé respirant',
        'Taille unique, épouse la forme de votre tête',
        'Bords larges pour une protection solaire optimale',
        'Protection UV : indice UPF 40+',
        'Disponible en 5 coloris exclusifs',
      ],
      care: [
        'Lavage à la main recommandé à 30°C',
        'Ne pas utiliser de sèche-linge',
        'Séchage à plat à l\'air libre',
        'Ne pas essorer ni tordre',
        'Ne pas repasser directement sur le tissu',
      ],
      order: 'COMMANDER',
      stockLabel: 'restants · édition limitée',
    },
    histoire: {
      title: 'Notre histoire',
      sections: [
        {
          title: 'Notre histoire',
          paragraphs: [
            "WYBOB est avant tout une histoire de famille. Tout a commencé par un cadeau. Mon frère a imaginé et dessiné un chapeau unique pour que nous le portions ensemble lors d'un festival. Un objet pensé pour être remarqué, faire sourire et rassembler.",
            "Très vite, nous avons compris que ce chapeau ne passait jamais inaperçu. Il attirait les regards, lançait les conversations et créait des rencontres. Plus qu'un simple accessoire, il incarnait déjà un état d'esprit : la joie de vivre.",
          ],
        },
        {
          kicker: "À l'origine de WYBOB",
          title: 'Pénélope Wybo',
          paragraphs: [
            "Je m'appelle Pénélope Wybo, fondatrice de WYBOB. J'ai grandi entre deux univers qui me définissent profondément : la mode et Madagascar. Issue d'une famille évoluant dans le textile depuis plus de 25 ans, j'ai développé très tôt une passion pour les matières, le savoir-faire artisanal et les créations qui racontent une histoire.",
            "Avec WYBOB, j'ai voulu créer bien plus qu'un chapeau. J'ai voulu donner vie à une marque qui célèbre la joie de vivre, met en lumière l'artisanat malgache et rassemble une communauté de personnes partageant les mêmes valeurs : l'authenticité, la liberté et le voyage.",
          ],
        },
        {
          title: 'Un savoir-faire qui a du sens',
          paragraphs: [
            "J'ai grandi dans l'univers du textile. Depuis toujours, je suis entourée de fils, de mailles, de crochet et de tricot. Cet héritage m'a appris à reconnaître la valeur d'un véritable savoir-faire artisanal et le temps nécessaire pour créer une pièce d'exception.",
          ],
          quote: "Nous croyons que le vrai luxe n'est pas celui qui fait du bruit. C'est celui qui possède une âme.",
          paragraphsAfterQuote: [
            "C'est naturellement qu'est née l'envie de faire vivre cette création. Mais WYBOB avait besoin d'une mission plus grande. Mes racines sont profondément liées à Madagascar, une terre dont le talent artisanal est immense et encore trop méconnu.",
            "Derrière chaque WYBOB se cachent des mains passionnées, des heures de travail minutieux et un savoir-faire transmis de génération en génération. À travers chaque création, nous voulons mettre en lumière ces artisans extraordinaires et faire rayonner Madagascar bien au-delà de ses frontières. Chaque WYBOB raconte l'histoire d'un artisan, d'une famille, d'un pays et d'une passion commune pour les belles choses.",
          ],
        },
      ] as HistoireSection[],
    },
    concept: {
      title: 'Produire moins, mais produire mieux',
      paragraphs: [
        'Chez WYBOB, nous avons choisi de faire les choses autrement. Nous croyons au slow fashion : produire moins, mais produire mieux. Chaque chapeau est fabriqué en édition limitée, au rythme de notre communauté, sans surproduction.',
        "Plutôt que de multiplier les modèles, nous avons fait le choix d'une seule silhouette. Un design intemporel, pensé pour traverser les années et devenir instantanément reconnaissable.",
        "Notre collection évolue à travers les couleurs. Chaque nouvelle édition ouvre un nouveau chapitre de l'histoire WYBOB, tandis que les précédentes deviennent des pièces uniques qui ne seront plus reproduites.",
        'Au-delà d\'un chapeau, WYBOB est une invitation à rejoindre une communauté de voyageurs, de créatifs et d\'optimistes qui partagent une même philosophie : vivre pleinement, créer des souvenirs et célébrer les moments qui comptent.',
      ],
      sections: [
        {
          title: 'Plus qu\'un chapeau',
          paragraphs: [
            "Le nom WYBOB n'a pas été choisi par hasard. Il représente ce lien familial qui est à l'origine de tout, mais aussi notre envie de créer une grande famille où chacun trouve sa place.",
            "Porter un WYBOB, ce n'est pas seulement porter un chapeau. C'est affirmer sa personnalité sans avoir besoin de parler. C'est choisir l'optimisme, la spontanéité et l'audace. C'est célébrer les festivals, les voyages, les couchers de soleil, les éclats de rire et tous ces instants qui rendent la vie plus belle.",
          ],
        },
        {
          title: 'Bienvenue dans la famille WYBOB',
          paragraphs: [
            "Aujourd'hui, notre ambition va bien au-delà de créer des chapeaux. Nous voulons construire une véritable communauté de personnes qui partagent les mêmes valeurs : la créativité, la liberté, le voyage, le partage et la joie de vivre.",
            "Chaque nouveau membre de la famille WYBOB participe à cette aventure et contribue à faire voyager l'artisanat malgache à travers le monde. Parce que les plus belles histoires sont celles qui se construisent ensemble.",
          ],
          closing: 'Bienvenue dans la famille WYBOB.',
        },
      ] as ConceptSection[],
      cta: 'Découvrir la collection',
    },
    galerie: {
      title: 'Notre Galerie',
      prevPage: 'Page précédente',
      nextPage: 'Page suivante',
      prev: 'précédent',
      next: 'suivant',
    },
    contact: {
      title: 'Contacts',
      name: 'Nom complet',
      email: 'Adresse email',
      phone: 'Numéro de téléphone (optionnel)',
      subject: 'Sujet',
      subjectOptions: [
        { value: 'produit', label: 'Question produit' },
        { value: 'sav', label: 'SAV / Retour' },
        { value: 'commande', label: 'Suivi de commande' },
        { value: 'autre', label: 'Autre' },
      ],
      orderNumber: 'Numéro de commande (optionnel)',
      message: 'Message',
      attachment: 'Photo (optionnel)',
      attachmentHint: 'JPG, PNG ou WEBP — 5 Mo max',
      attachmentRemove: 'Retirer',
      send: 'ENVOYER',
      sending: 'ENVOI EN COURS...',
      success: 'Votre message a bien été envoyé, merci !',
      error: 'Une erreur est survenue, veuillez réessayer.',
      networkError: 'Impossible de contacter le serveur. Vérifiez votre connexion.',
      nameRequired: 'Le nom est requis',
      emailInvalid: 'Adresse email invalide',
      messageRequired: 'Le message est requis',
      fileTooLarge: 'Image trop lourde (5 Mo max)',
      fileInvalidType: 'Format non supporté (JPG, PNG ou WEBP uniquement)',
    },
    cart: {
      empty: 'Votre panier est vide 🛒',
      backToShop: 'Retour à la boutique',
      back: 'Retour',
      title: 'Mon panier',
      itemSubtitle: 'Cette maille est vraiment trop cool',
      colorLabel: 'Couleur :',
      decreaseQty: 'Diminuer la quantité',
      increaseQty: 'Augmenter la quantité',
      remove: 'Retirer',
      removeFromCart: 'Retirer du panier',
      promoTitle: 'Code promo',
      promoPlaceholder: 'Entre ton code promo',
      promoApply: 'Appliquer',
      promoApplying: '...',
      promoRemove: 'Retirer le code',
      promoInvalid: 'Code invalide',
      promoError: 'Erreur lors de la vérification',
      summaryTitle: 'Résumé de la commande',
      subtotal: 'Sous-total',
      discount: 'Réduction',
      total: 'Total',
      checkout: 'Procéder au paiement',
    },
    checkout: {
      loading: 'Chargement...',
      stripeUnavailable: 'Le paiement en ligne sera disponible prochainement.',
      back: 'Retour',
      contactTitle: 'Contact',
      emailPlaceholder: 'Tom.exemple@gmail.com',
      phonePlaceholder: 'Téléphone (ex: 034 00 000 00)',
      deliveryTitle: 'Livraison',
      useSavedAddress: 'Utiliser une adresse enregistrée…',
      addressFallback: 'Adresse',
      firstnamePlaceholder: 'Prénom',
      lastnamePlaceholder: 'Nom',
      companyPlaceholder: 'Entreprise (optionnel)',
      addressPlaceholder: 'Adresse',
      postalCodePlaceholder: 'Code postal',
      cityPlaceholder: 'Ville',
      shippingMethodTitle: "Mode d'expédition",
      pickupNamePlaceholder: 'Personne habilitée à retirer le colis (optionnel)',
      insuranceLabel: 'Assurer mon colis pour sa valeur totale',
      insuranceIncludedHint: 'Chronopost couvre déjà chaque colis jusqu\'à {included}€ gratuitement.',
      insuranceExtendHint: 'Cette option étend la couverture jusqu\'à {value}€ en cas de perte ou d\'avarie, pour +{fee}€.',
      paymentTitle: 'Paiement',
      submitting: 'Envoi...',
      submit: 'Vérifier la commande',
      summaryTitle: 'Résumé de la commande',
      subtotal: 'Sous-total',
      discount: 'Réduction',
      deliveryLabel: 'Livraison',
      insuranceLine: 'Assurance colis',
      total: 'Total',
      errors: {
        requiredFields: 'Veuillez remplir tous les champs obligatoires',
        invalidPostalCode: 'Le code postal ne correspond pas au format attendu pour ce pays',
        relayPointRequired: 'Veuillez sélectionner un point relais',
        emptyCart: 'Votre panier est vide',
        payment: 'Erreur de paiement',
        paymentIntent: 'Erreur lors de la création du paiement',
        order: 'Erreur lors de la commande',
        server: 'Erreur serveur',
      },
      relay: {
        findButton: 'Trouver un point relais',
        findAgain: 'Rechercher à nouveau',
        searching: 'Recherche…',
        missingFields: "Renseignez d'abord le code postal et la ville ci-dessus",
        unavailable: 'Recherche indisponible',
        noneFound: 'Aucun point relais trouvé à proximité',
        serverError: 'Erreur serveur',
        selectedHint: 'Point relais sélectionné :',
        radiusLabel: 'Rayon',
        customerMarkerLabel: 'Votre adresse',
      },
      success: {
        failedTitle: 'Paiement non abouti',
        failedText: "Le paiement a été annulé ou refusé. Votre commande n'a pas été validée.",
        retry: 'Réessayer',
        successTitle: 'Paiement reçu',
        successText: 'Votre paiement a bien été validé. Vous recevrez un email de confirmation sous peu.',
        backHome: "Retour à l'accueil",
      },
      countries: {
        AL: 'Albanie', DE: 'Allemagne', AD: 'Andorre', AT: 'Autriche', BE: 'Belgique',
        BA: 'Bosnie-Herzégovine', BG: 'Bulgarie', CY: 'Chypre', HR: 'Croatie', DK: 'Danemark',
        ES: 'Espagne', EE: 'Estonie', FI: 'Finlande', FR: 'France', GR: 'Grèce',
        HU: 'Hongrie', IE: 'Irlande', IS: 'Islande', IT: 'Italie', LV: 'Lettonie',
        LI: 'Liechtenstein', LT: 'Lituanie', LU: 'Luxembourg', MK: 'Macédoine du Nord', MT: 'Malte',
        MC: 'Monaco', ME: 'Monténégro', NO: 'Norvège', NL: 'Pays-Bas', PL: 'Pologne',
        PT: 'Portugal', CZ: 'République tchèque', RO: 'Roumanie', GB: 'Royaume-Uni', SM: 'Saint-Marin',
        RS: 'Serbie', SK: 'Slovaquie', SI: 'Slovénie', SE: 'Suède', CH: 'Suisse',
        UA: 'Ukraine', VA: 'Vatican',
      },
    },
    login: {
      welcome: 'Ravi de vous revoir',
      emailLabel: 'Adresse e-mail',
      passwordLabel: 'Mot de passe',
      remember: 'Se souvenir de moi',
      forgot: 'Mot de passe oublié',
      submit: 'Se connecter',
      loading: 'Connexion...',
      noAccount: 'Pas encore de compte ?',
      createAccount: 'Créer un compte',
      errors: {
        fill: 'Veuillez remplir tous les champs',
        invalidEmail: 'Adresse e-mail invalide',
        weakPassword: 'Le mot de passe doit contenir au moins 8 caractères, une lettre et un chiffre',
        invalid: 'Email ou mot de passe incorrect',
        accountLocked: 'Compte verrouillé après plusieurs tentatives. Réessayez dans quelques minutes.',
        emailNotVerified: 'Email non vérifié. Consultez votre boîte mail.',
      },
      resendVerification: 'Renvoyer l\'email de vérification',
      resendVerificationSending: 'Envoi...',
      resendVerificationSent: 'Email de vérification renvoyé, consultez votre boîte mail.',
    },
    register: {
      welcome: 'Créer votre compte',
      nameLabel: 'Nom complet',
      emailLabel: 'Adresse e-mail',
      passwordLabel: 'Mot de passe',
      confirmLabel: 'Confirmer le mot de passe',
      submit: 'Créer mon compte',
      loading: 'Création...',
      hasAccount: 'Vous avez déjà un compte ?',
      login: 'Se connecter',
      success: 'Compte créé ! Vérifiez votre email avant de vous connecter.',
      hide: 'Masquer',
      show: 'Afficher',
      errors: {
        nameLength: 'Le nom doit contenir entre 2 et 50 caractères',
        invalidEmail: 'Adresse email invalide',
        noMatch: 'Les mots de passe ne correspondent pas',
        server: 'Erreur serveur',
      },
      strength: {
        weak: 'Faible',
        medium: 'Moyen',
        strong: 'Fort',
      },
      validation: {
        minChars: '8 caractères minimum',
        needLetter: 'au moins une lettre',
        needNumber: 'au moins un chiffre',
      },
    },
    dashboard: {
      sidebar: {
        overview: "Vue d'ensemble",
        orders: "Commandes",
        profile: "Profil",
        addresses: "Adresses",
        referral: "Parrainage",
        logout: "Déconnexion",
        exit: "Sortir",
        more: "Plus",
      },
      overview: {
        title: "Vue d'ensemble",
        kpiOrders: "Commandes",
        kpiTotal: "Total dépensé",
        kpiActive: "En cours",
        kpiTotalSub: "au total",
        kpiAllOrders: "toutes commandes",
        activeOrder: "commande active",
        activeOrders: "commandes actives",
        recentOrders: "Dernières commandes",
        seeAll: "Tout voir →",
        noOrders: "Aucune commande pour le moment.",
        discover: "Découvrir la boutique",
        myAddress: "Mon adresse",
        manage: "Gérer →",
        noAddress: "Aucune adresse enregistrée.",
        addAddress: "+ Ajouter une adresse",
        delivery: "Livraison",
        moreOrders: "autres commandes",
        article: "article",
        articles: "articles",
      },
      orders: {
        title: "Mes commandes",
        noOrders: "Aucune commande pour le moment.",
        cancelled: "Cette commande a été annulée.",
        delivery: "Livraison :",
        payment: "Paiement :",
        articles: "Articles :",
        address: "Adresse :",
        productRemoved: "Produit supprimé",
        downloadInvoice: "Facture",
        statuses: {
          pending: "En attente",
          paid: "Payée",
          processing: "En préparation",
          shipped: "Expédiée",
          delivered: "Livrée",
          cancelled: "Annulée",
        },
        timeline: {
          pending: "En attente",
          paid: "Payée",
          processing: "Préparation",
          shipped: "Expédiée",
          delivered: "Livrée",
        },
        paymentMethods: {
          cash: "Espèces à la livraison",
          mobile_money: "Mobile Money",
          card: "Carte bancaire",
          bank_transfer: "Virement bancaire",
        },
        deliveryMethods: {
          standard: "Livraison standard",
          express: "Livraison express",
          pickup: "Retrait en magasin",
          colissimo: "Chronopost",
          relais: "Point relais Chronopost",
        },
      },
      profile: {
        title: "Mon profil",
        personalInfo: "Informations personnelles",
        fullName: "Nom complet",
        email: "Email",
        phone: "Téléphone",
        emailHint: "L'email ne peut pas être modifié.",
        save: "Enregistrer",
        saving: "Enregistrement…",
        changePassword: "Changer le mot de passe",
        currentPassword: "Mot de passe actuel",
        newPassword: "Nouveau mot de passe",
        confirmPassword: "Confirmer le mot de passe",
        changeBtn: "Modifier le mot de passe",
        changing: "Modification…",
        errors: {
          nameEmpty: "Le nom ne peut pas être vide.",
          noMatch: "Les mots de passe ne correspondent pas.",
          network: "Erreur réseau.",
        },
        success: {
          profile: "Profil mis à jour.",
          password: "Mot de passe modifié.",
        },
      },
      addresses: {
        title: "Mes adresses",
        sectionTitle: "Adresses de livraison",
        noAddresses: "Aucune adresse enregistrée.",
        addBtn: "+ Ajouter une adresse",
        modalTitle: "Nouvelle adresse",
        labelPlaceholder: "Nom de l'adresse (ex : Domicile)",
        fullNamePlaceholder: "Nom complet *",
        streetPlaceholder: "Adresse (numéro et nom de rue) *",
        zipPlaceholder: "Code postal",
        cityPlaceholder: "Ville *",
        countryPlaceholder: "Pays *",
        defaultLabel: "Adresse n°",
        save: "Enregistrer",
        saving: "Enregistrement…",
      },
    },
  },
  en: {
    nav: {
      home: 'Our Bob',
      histoire: 'Our Story',
      concept: 'Our Concept',
      galerie: 'Gallery',
      contact: 'Contact',
      panier: 'Cart',
      login: 'Sign in',
      register: 'Create account',
      dashboard: 'My account',
      logout: 'Sign out',
    },
    footer: {
      legal: 'Legal notice',
      cgv: 'T&Cs',
      privacy: 'Privacy policy',
      madeByPrefix: 'Made by ',
      manageCookies: 'Manage cookies',
    },
    cookieConsent: {
      message: "We use essential cookies to run the site and, with your consent, analytics cookies to measure audience. You can change your choice at any time.",
      customize: 'Customize',
      rejectAll: 'Reject all',
      acceptAll: 'Accept all',
      settingsTitle: 'Cookie preferences',
      essentialTitle: 'Essential cookies',
      essentialDesc: 'Required for the site to work (cart, session, authentication). Always active.',
      analyticsTitle: 'Analytics cookies',
      analyticsDesc: 'Google Analytics — anonymized audience measurement. Only set with your consent.',
      save: 'Save my choices',
      privacyLink: 'Learn more in our privacy policy',
      close: 'Close',
    },
    home: {
      tagline: "Thought through down to the smallest detail, the WYBOB blends character, comfort and artisanal craftsmanship.",
      description: 'Designed for bright days and summer escapes, the WYBOB Soleil brings a natural elegance to every outfit.',
      colors: {
        Blue: 'Blue',
        White: 'White',
        Yellow: 'Yellow',
        Red: 'Red',
      },
      colorStories: {
        Jaune: { name: 'MASOANDRO', text: "Light. A sunny color that catches the eye and brings a touch of good mood to every moment." },
        Bleu:  { name: 'RANO',       text: "The horizon. Inspired by the sky and the ocean, a blue that evokes freedom, calm and escape." },
        Rouge: { name: 'AFO',        text: "Intensity. A vibrant color that asserts your presence with character and confidence." },
        Blanc: { name: 'RAHONA',     text: "The essential. A timeless color that transcends seasons and leaves room for your story." },
        Barea: { name: '',          text: "A signature edition created to honor WYBOB's roots and celebrate the heritage, colors and artisanal craftsmanship of Madagascar." },
      },
      colorLabel: 'Current color:',
      sectionDescription: 'Description',
      sectionColor: 'Color',
      sectionFeatures: 'Features',
      sectionCare: 'Care instructions',
      features: [
        'Material: 100% organic cotton, breathable weave',
        'One size, shapes to the form of your head',
        'Wide brim for optimal sun protection',
        'UV protection: UPF 40+ rating',
        'Available in 5 exclusive colors',
      ],
      care: [
        'Hand wash recommended at 30°C',
        'Do not use a tumble dryer',
        'Lay flat to dry in the open air',
        'Do not wring or twist',
        'Do not iron directly on the fabric',
      ],
      order: 'ORDER',
      stockLabel: 'left · limited edition',
    },
    histoire: {
      title: 'Our story',
      sections: [
        {
          title: 'Our story',
          paragraphs: [
            "WYBOB is, above all, a family story. It all began with a gift. My brother imagined and designed a one-of-a-kind hat for us to wear together at a festival. An object made to be noticed, to spark smiles and bring people together.",
            "We quickly realised this hat never went unnoticed. It turned heads, started conversations, created encounters. More than a simple accessory, it already embodied a state of mind: joie de vivre.",
          ],
        },
        {
          kicker: 'The origin of WYBOB',
          title: 'Pénélope Wybo',
          paragraphs: [
            "My name is Pénélope Wybo, founder of WYBOB. I grew up between two worlds that define me deeply: fashion and Madagascar. Coming from a family that has worked in textiles for over 25 years, I developed an early passion for materials, artisanal know-how and creations that tell a story.",
            "With WYBOB, I wanted to create far more than a hat. I wanted to bring to life a brand that celebrates joie de vivre, shines a light on Malagasy craftsmanship, and brings together a community sharing the same values: authenticity, freedom and travel.",
          ],
        },
        {
          title: 'A craft that means something',
          paragraphs: [
            "I grew up surrounded by thread, stitches, crochet and knitting. This heritage taught me to recognise true artisanal know-how and the time it takes to create an exceptional piece.",
          ],
          quote: "We believe real luxury isn't loud. It has a soul.",
          paragraphsAfterQuote: [
            "That's how the desire to bring this creation to life naturally came about. But WYBOB needed a bigger mission. My roots run deep in Madagascar, a land of immense — and still largely unknown — artisanal talent.",
            "Behind every WYBOB are passionate hands, hours of meticulous work, and know-how passed down through generations. Through every creation, we want to shine a light on these extraordinary artisans and let Madagascar shine far beyond its borders. Every WYBOB tells the story of an artisan, a family, a country, and a shared passion for beautiful things.",
          ],
        },
      ] as HistoireSection[],
    },
    concept: {
      title: 'Our concept',
      paragraphs: [
        'At WYBOB, we chose to do things differently. We believe in slow fashion: producing less, but producing better. Every hat is made in a limited edition, at the pace of our community, without overproduction.',
        'Rather than multiplying models, we chose a single silhouette — a timeless design, made to last through the years and become instantly recognisable.',
        'Our collection evolves through colour. Each new edition opens a new chapter in the WYBOB story, while previous ones become unique pieces that will never be reproduced again.',
        'Beyond a hat, WYBOB is an invitation to join a community of travellers, creatives and optimists who share one philosophy: living fully, creating memories, and celebrating the moments that matter.',
      ],
      sections: [
        {
          title: 'More than a hat',
          paragraphs: [
            "The name WYBOB wasn't chosen by chance. It represents the family bond at the origin of everything — and our wish to build one big family where everyone finds their place.",
            "Wearing a WYBOB isn't just wearing a hat. It's asserting your personality without having to speak. It's choosing optimism, spontaneity and boldness. It's celebrating festivals, travels, sunsets, laughter — all the moments that make life more beautiful.",
          ],
        },
        {
          title: 'Welcome to the WYBOB family',
          paragraphs: [
            'Today, our ambition goes far beyond making hats. We want to build a true community of people who share the same values: creativity, freedom, travel, sharing and joie de vivre.',
            "Every new member of the WYBOB family takes part in this adventure and helps Malagasy craftsmanship travel the world. Because the most beautiful stories are the ones we build together.",
          ],
          closing: 'Welcome to the WYBOB family.',
        },
      ] as ConceptSection[],
      cta: 'Discover the collection',
    },
    galerie: {
      title: 'Our Gallery',
      prevPage: 'Previous page',
      nextPage: 'Next page',
      prev: 'previous',
      next: 'next',
    },
    contact: {
      title: 'Contact',
      name: 'Full name',
      email: 'Email address',
      phone: 'Phone number (optional)',
      subject: 'Subject',
      subjectOptions: [
        { value: 'produit', label: 'Product question' },
        { value: 'sav', label: 'Support / Return' },
        { value: 'commande', label: 'Order tracking' },
        { value: 'autre', label: 'Other' },
      ],
      orderNumber: 'Order number (optional)',
      message: 'Message',
      attachment: 'Photo (optional)',
      attachmentHint: 'JPG, PNG or WEBP — 5 MB max',
      attachmentRemove: 'Remove',
      send: 'SEND',
      sending: 'SENDING...',
      success: 'Your message has been sent, thank you!',
      error: 'Something went wrong, please try again.',
      networkError: 'Could not reach the server. Check your connection.',
      nameRequired: 'Name is required',
      emailInvalid: 'Invalid email address',
      messageRequired: 'Message is required',
      fileTooLarge: 'Image too large (5 MB max)',
      fileInvalidType: 'Unsupported format (JPG, PNG or WEBP only)',
    },
    cart: {
      empty: 'Your cart is empty 🛒',
      backToShop: 'Back to shop',
      back: 'Back',
      title: 'My cart',
      itemSubtitle: 'This knit is really cool',
      colorLabel: 'Color:',
      decreaseQty: 'Decrease quantity',
      increaseQty: 'Increase quantity',
      remove: 'Remove',
      removeFromCart: 'Remove from cart',
      promoTitle: 'Promo code',
      promoPlaceholder: 'Enter your promo code',
      promoApply: 'Apply',
      promoApplying: '...',
      promoRemove: 'Remove code',
      promoInvalid: 'Invalid code',
      promoError: 'Error while checking the code',
      summaryTitle: 'Order summary',
      subtotal: 'Subtotal',
      discount: 'Discount',
      total: 'Total',
      checkout: 'Proceed to checkout',
    },
    checkout: {
      loading: 'Loading...',
      stripeUnavailable: 'Online payment will be available soon.',
      back: 'Back',
      contactTitle: 'Contact',
      emailPlaceholder: 'Tom.example@gmail.com',
      phonePlaceholder: 'Phone (e.g. 034 00 000 00)',
      deliveryTitle: 'Delivery',
      useSavedAddress: 'Use a saved address…',
      addressFallback: 'Address',
      firstnamePlaceholder: 'First name',
      lastnamePlaceholder: 'Last name',
      companyPlaceholder: 'Company (optional)',
      addressPlaceholder: 'Address',
      postalCodePlaceholder: 'Postal code',
      cityPlaceholder: 'City',
      shippingMethodTitle: 'Shipping method',
      pickupNamePlaceholder: 'Person authorized to collect the parcel (optional)',
      insuranceLabel: 'Insure my parcel for its full value',
      insuranceIncludedHint: 'Chronopost already covers every parcel up to {included}€ for free.',
      insuranceExtendHint: 'This option extends coverage up to {value}€ in case of loss or damage, for +{fee}€.',
      paymentTitle: 'Payment',
      submitting: 'Sending...',
      submit: 'Review order',
      summaryTitle: 'Order summary',
      subtotal: 'Subtotal',
      discount: 'Discount',
      deliveryLabel: 'Delivery',
      insuranceLine: 'Parcel insurance',
      total: 'Total',
      errors: {
        requiredFields: 'Please fill in all required fields',
        invalidPostalCode: 'The postal code does not match the expected format for this country',
        relayPointRequired: 'Please select a relay point',
        emptyCart: 'Your cart is empty',
        payment: 'Payment error',
        paymentIntent: 'Error while creating the payment',
        order: 'Error while placing the order',
        server: 'Server error',
      },
      relay: {
        findButton: 'Find a relay point',
        findAgain: 'Search again',
        searching: 'Searching…',
        missingFields: 'Please enter the postal code and city above first',
        unavailable: 'Search unavailable',
        noneFound: 'No relay point found nearby',
        serverError: 'Server error',
        selectedHint: 'Selected relay point:',
        radiusLabel: 'Radius',
        customerMarkerLabel: 'Your address',
      },
      success: {
        failedTitle: 'Payment unsuccessful',
        failedText: 'The payment was cancelled or declined. Your order has not been placed.',
        retry: 'Try again',
        successTitle: 'Payment received',
        successText: "Your payment has been confirmed. You'll receive a confirmation email shortly.",
        backHome: 'Back to home',
      },
      countries: {
        AL: 'Albania', DE: 'Germany', AD: 'Andorra', AT: 'Austria', BE: 'Belgium',
        BA: 'Bosnia and Herzegovina', BG: 'Bulgaria', CY: 'Cyprus', HR: 'Croatia', DK: 'Denmark',
        ES: 'Spain', EE: 'Estonia', FI: 'Finland', FR: 'France', GR: 'Greece',
        HU: 'Hungary', IE: 'Ireland', IS: 'Iceland', IT: 'Italy', LV: 'Latvia',
        LI: 'Liechtenstein', LT: 'Lithuania', LU: 'Luxembourg', MK: 'North Macedonia', MT: 'Malta',
        MC: 'Monaco', ME: 'Montenegro', NO: 'Norway', NL: 'Netherlands', PL: 'Poland',
        PT: 'Portugal', CZ: 'Czech Republic', RO: 'Romania', GB: 'United Kingdom', SM: 'San Marino',
        RS: 'Serbia', SK: 'Slovakia', SI: 'Slovenia', SE: 'Sweden', CH: 'Switzerland',
        UA: 'Ukraine', VA: 'Vatican City',
      },
    },
    login: {
      welcome: 'Welcome back',
      emailLabel: 'Email address',
      passwordLabel: 'Password',
      remember: 'Remember me',
      forgot: 'Forgot password',
      submit: 'Sign in',
      loading: 'Signing in...',
      noAccount: "Don't have an account?",
      createAccount: 'Create account',
      errors: {
        fill: 'Please fill in all fields',
        invalidEmail: 'Invalid email address',
        weakPassword: 'Password must be at least 8 characters, include a letter and a number',
        invalid: 'Incorrect email or password',
        accountLocked: 'Account locked after several attempts. Please try again in a few minutes.',
        emailNotVerified: 'Email not verified. Check your inbox.',
      },
      resendVerification: 'Resend verification email',
      resendVerificationSending: 'Sending...',
      resendVerificationSent: 'Verification email resent, check your inbox.',
    },
    register: {
      welcome: 'Create your account',
      nameLabel: 'Full name',
      emailLabel: 'Email address',
      passwordLabel: 'Password',
      confirmLabel: 'Confirm password',
      submit: 'Create account',
      loading: 'Creating...',
      hasAccount: 'Already have an account?',
      login: 'Sign in',
      success: 'Account created! Please verify your email before signing in.',
      hide: 'Hide',
      show: 'Show',
      errors: {
        nameLength: 'Name must be between 2 and 50 characters',
        invalidEmail: 'Invalid email address',
        noMatch: 'Passwords do not match',
        server: 'Server error',
      },
      strength: {
        weak: 'Weak',
        medium: 'Medium',
        strong: 'Strong',
      },
      validation: {
        minChars: 'at least 8 characters',
        needLetter: 'at least one letter',
        needNumber: 'at least one number',
      },
    },
    dashboard: {
      sidebar: {
        overview: "Overview",
        orders: "Orders",
        profile: "Profile",
        addresses: "Addresses",
        referral: "Referral",
        logout: "Sign out",
        exit: "Exit",
        more: "More",
      },
      overview: {
        title: "Overview",
        kpiOrders: "Orders",
        kpiTotal: "Total spent",
        kpiActive: "Active",
        kpiTotalSub: "in total",
        kpiAllOrders: "all orders",
        activeOrder: "active order",
        activeOrders: "active orders",
        recentOrders: "Recent orders",
        seeAll: "See all →",
        noOrders: "No orders yet.",
        discover: "Explore the shop",
        myAddress: "My address",
        manage: "Manage →",
        noAddress: "No address saved.",
        addAddress: "+ Add an address",
        delivery: "Delivery",
        moreOrders: "more orders",
        article: "item",
        articles: "items",
      },
      orders: {
        title: "My orders",
        noOrders: "No orders yet.",
        cancelled: "This order has been cancelled.",
        delivery: "Delivery:",
        payment: "Payment:",
        articles: "Items:",
        address: "Address:",
        productRemoved: "Removed product",
        downloadInvoice: "Invoice",
        statuses: {
          pending: "Pending",
          paid: "Paid",
          processing: "Processing",
          shipped: "Shipped",
          delivered: "Delivered",
          cancelled: "Cancelled",
        },
        timeline: {
          pending: "Pending",
          paid: "Paid",
          processing: "Processing",
          shipped: "Shipped",
          delivered: "Delivered",
        },
        paymentMethods: {
          cash: "Cash on delivery",
          mobile_money: "Mobile Money",
          card: "Credit card",
          bank_transfer: "Bank transfer",
        },
        deliveryMethods: {
          standard: "Standard delivery",
          express: "Express delivery",
          pickup: "In-store pickup",
          colissimo: "Chronopost",
          relais: "Chronopost relay point",
        },
      },
      profile: {
        title: "My profile",
        personalInfo: "Personal information",
        fullName: "Full name",
        email: "Email",
        phone: "Phone",
        emailHint: "Email cannot be changed.",
        save: "Save",
        saving: "Saving…",
        changePassword: "Change password",
        currentPassword: "Current password",
        newPassword: "New password",
        confirmPassword: "Confirm password",
        changeBtn: "Change password",
        changing: "Changing…",
        errors: {
          nameEmpty: "Name cannot be empty.",
          noMatch: "Passwords do not match.",
          network: "Network error.",
        },
        success: {
          profile: "Profile updated.",
          password: "Password changed.",
        },
      },
      addresses: {
        title: "My addresses",
        sectionTitle: "Delivery addresses",
        noAddresses: "No address saved.",
        addBtn: "+ Add an address",
        modalTitle: "New address",
        labelPlaceholder: "Address name (e.g. Home)",
        fullNamePlaceholder: "Full name *",
        streetPlaceholder: "Street address *",
        zipPlaceholder: "Postal code",
        cityPlaceholder: "City *",
        countryPlaceholder: "Country *",
        defaultLabel: "Address #",
        save: "Save",
        saving: "Saving…",
      },
    },
  },
} as const
